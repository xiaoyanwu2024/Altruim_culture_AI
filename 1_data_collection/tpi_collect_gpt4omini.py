...
# Author : Xiaoyan Wu
# Date   : June 2026
# Description: TPI data collection — GPT-4o-mini (direct probability elicitation)
# =============================================================================

"""
GPT-4o-mini Task 2 — direct probability elicitation.
Model outputs 0-100 integer per trial.
8 countries x 2 genders x 300 conditions = 4800 requests.

Usage:
    export OPENAI_API_KEY="..."
    python3 collect_gpt4omini_prob.py          # full run
    python3 collect_gpt4omini_prob.py test     # test 5 rows only
"""
...
TP_TOKENS  = 50
MAX_WORKERS = 60   # GPT-4o-mini has generous rate limits
...
def call_one(client, country, gender, cond):
    ...
    for attempt in range(4):
        try:
            ...
            print(f"  [gpt4omini] retry {attempt+1}: {err[:80]} — wait {wait}s")
            time.sleep(wait)
    return None
...
def main(test=False):
    ...
    total = len(tasks)
    out_path = DATA_DIR / f"gpt4omini_prob_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
