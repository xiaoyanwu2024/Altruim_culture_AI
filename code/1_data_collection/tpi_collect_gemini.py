...
# Author : Xiaoyan Wu
# Date   : June 2026
# Description: TPI data collection — Gemini-2.5-Flash (direct probability elicitation)
# =============================================================================

"""
Gemini 2.5 Flash Task 2 — Third-Party Punishment.
Direct probability elicitation: model outputs 0-100 integer.
8 countries x 2 genders x 300 conditions = 4800 requests.

Usage:
    export GEMINI_API_KEY="..."
    python3 collect_gemini.py          # full run
    python3 collect_gemini.py test     # test 5 rows only
"""
...
# ── Prompts (identical system prompt to all other models) ──────────────────────
...
def call_one(client, country, gender, cond):
    ...
    for attempt in range(4):
        try:
            ...
            raw = r.text.strip() if r.text else None
            return parse_prob(raw) if raw else None
        except Exception as e:
            err = str(e)
            wait = max(15, 2 ** attempt) if "429" in err or "quota" in err.lower() else 2 ** attempt
            print(f"  [gemini] retry {attempt+1}: {err[:80]} — wait {wait}s")
            time.sleep(wait)
    return None
...
def main(test=False):
    ...
    total = len(tasks)
    out_path = DATA_DIR / f"gemini25flash_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
