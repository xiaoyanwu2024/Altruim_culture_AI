...
# Author : Xiaoyan Wu
# Date   : June 2026
# Description: TPI data collection — Gemini-3.5-Flash (direct probability elicitation)
# =============================================================================

"""
Gemini 3.5 Flash Task 2 — direct probability elicitation (0-100 integer).
8 countries x 2 genders x 300 conditions = 4800 requests.
Retries indefinitely per trial until valid output obtained.
"""
...
def call_until_valid(client, country, gender, cond):
    ...
    while True:
        ...
        try:
            ...
            err = str(e)
            wait = max(15, min(2**attempt, 60)) if "429" in err or "quota" in err.lower() else min(2**attempt, 30)
            time.sleep(wait)
            continue
        time.sleep(2)
...
def main():
    ...
    def run(args):
        ...
        p = call_until_valid(client, country, gender, cond)
        with lock:
            done_count[0] += 1
            if done_count[0] % 200 == 0:
                print(f"  {done_count[0]}/{total} done")
        return {
    ...
    out = DATA_DIR / f"gemini35flash_prob_{ts}.csv"
