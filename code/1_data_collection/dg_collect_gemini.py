...
# Author : Xiaoyan Wu
# Date   : June 2026
# Description: DG data collection — Gemini-2.5-Flash
# =============================================================================

"""
Gemini 2.5 Flash Task 1 — Dictator Game (Allocator role).
...
def main():
    ...
    out_path = DATA_DIR / f"gemini25flash_task1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            ...
            for gender in GENDERS:
                ...
                for attempt in range(4):
                    try:
                        ...
                        err = str(e)
                        if "429" in err or "RATE" in err.upper() or "quota" in err.lower():
                            wait = max(15, 2 ** attempt)
                        else:
                            wait = 2 ** attempt
                        print(f"  retry {attempt+1}: {err[:80]} — wait {wait}s")
                        time.sleep(wait)
                ...
                f.flush()
                time.sleep(0.5)  # paid tier, minimal delay
