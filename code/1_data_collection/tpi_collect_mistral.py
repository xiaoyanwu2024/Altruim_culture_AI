...
# Author : Xiaoyan Wu
# Date   : June 2026
# Description: TPI data collection — Mistral-Small-4 (direct probability elicitation)
# =============================================================================

"""
Mistral Small 4 Task 2 — Third-Party Punishment.
Direct probability elicitation: model outputs 0-100 integer.
8 countries x 2 genders x 300 conditions = 4800 requests.

Usage:
    export MISTRAL_API_KEY="..."
    python3 collect_mistral.py          # full run
    python3 collect_mistral.py test     # test 5 rows only
"""
...
def call_one(client, country, gender, cond):
    ...
    for attempt in range(8):
        try:
            ...
            err = str(e)
            wait = 60 if "429" in err or "rate" in err.lower() else 2 ** attempt
            print(f"  [mistral] retry {attempt+1}: {err[:80]} — wait {wait}s")
            time.sleep(wait)
    return None
...
def main(test=False):
    ...
    total = len(tasks)
    out_path = DATA_DIR / f"mistral_small4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
