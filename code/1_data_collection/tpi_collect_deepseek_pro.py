...
# Author : Xiaoyan Wu
# Date   : June 2026
# Description: TPI data collection — DeepSeek-V4-Pro (direct probability elicitation)
# =============================================================================

"""
DeepSeek V4-Pro (non-thinking) Task 2 — direct probability elicitation.
Model outputs 0-100 integer per trial.
8 countries x 2 genders x 300 conditions = 4800 requests.

Usage:
    export DEEPSEEK_API_KEY="..."
    python3 collect_deepseek_pro_prob.py          # full run
    python3 collect_deepseek_pro_prob.py test     # test 5 rows only
"""
...
def call_one(client, country, gender, cond):
    ...
    for attempt in range(4):
        try:
            ...
            wait = 2 ** attempt
            print(f"  [deepseek-pro] retry {attempt+1}: {str(e)[:80]} — wait {wait}s")
            time.sleep(wait)
    return None
...
def main(test=False):
    ...
    total = len(tasks)
    out_path = DATA_DIR / f"deepseek_v4pro_prob_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    fields = ["model","country","gender","cond_id","block_type","x1","x2","cost","ratio","p_yes"]

    print(f"Model: {MODEL_ID}  (direct probability elicitation, thinking disabled)")
