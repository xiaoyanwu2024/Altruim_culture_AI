...
% Author : Xiaoyan Wu
% Date   : June 2026
% Description: Model M6 — adds relative payoff (RP)
% =============================================================================

function lik = m6(x,data)
...
%
% Usage:
%   Computes the log likelihood of the SI + SCI + VCI + EC + RP model (model 6)
%
% Inputs:
...
for t = 1:length(data.block)
    ...
    Uyes = x3s - envy*disad - guilt*ad - gama*inqua + oumiga*EP + kapa*RP; % utility of "yes"
    Uno = x3 - envy*(max(x1-x3,0)+max(x2-x3,0)) - guilt*(max(x3-x1,0)+max(x3-x2,0)) - gama*max(x1-x2,0)  + oumiga*(x1+x2) + kapa*max(x2-x1,0); % utility of "no"
