...
% Author : Xiaoyan Wu
% Date   : June 2026
% Description: Model M5 — adds empathy/cost sensitivity (EC)
% =============================================================================

function lik = m5(x,data)
...
%
% Usage:
%   Computes the log likelihood of the SI + SCI + VCI + EC model (model 5)
%
% Inputs:
...
for t = 1:length(data.block)
    ...
    Uyes = x3s - envy*disad - guilt*ad - gama*inqua + oumiga*EP; % utility of "yes"
    Uno = x3 - envy*(max(x1-x3,0)+max(x2-x3,0)) - guilt*(max(x3-x1,0)+max(x3-x2,0)) - gama*max(x1-x2,0)  + oumiga*(x1+x2); % utility of "no"
